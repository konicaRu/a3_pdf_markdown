...
План обработки (на каждую страницу):

1. Проанализировать страницу через PyMuPDF.
2. **Попробовать `page.find_tables()`**: если найдены таблицы — конвертировать их в Markdown-таблицы, вытащить текст вне таблиц по bbox-фильтру, собрать результат и перейти к следующей странице (OCR и vision не используются).
3. Если таблиц нет: проверить текстовый слой. Если хороший — извлечь текст без OCR.
4. Если текстовый слой слабый или отсутствует — использовать OCR.
5. Найти страницы/области с графиками, схемами, диаграммами, изображениями или низким качеством OCR.
6. Для таких страниц вызвать vision-модель.
7. Собрать Markdown из текста OCR/текстового слоя и точечных описаний визуальных элементов.
8. Если OCR полностью не справился на странице — использовать vision fallback для этой страницы.
...
- Отдельный PDF pipeline:
  - **структурное извлечение таблиц через `page.find_tables()`** (PyMuPDF): если на странице найдены таблицы, они конвертируются в Markdown напрямую без OCR и vision; текст вне таблиц (заголовки и т.п.) извлекается отдельно по bbox-фильтру;
  - текстовый слой через PyMuPDF — для страниц без таблиц;
  - EasyOCR, если текстовый слой слабый;
  - vision только для визуальных страниц/слабого OCR (не задействуется, если таблицы найдены);
  - LM Studio как OpenAI-compatible endpoint;
  - Ollama через `/api/chat`.

## Ближайшие технические задачи

- Продолжить ручную проверку GUI на реальном Windows-окружении.
- Проверить один маленький DOCX, один PDF-скан и несколько PPTX с разными типами картинок.
...
Для ежедневного запуска в разработке используется:

```text
A3 PDF Markdown.vbs
```

Если нужно увидеть ошибку запуска:

```text
A3 PDF Markdown.cmd
```

Полноценный `.exe` остается следующей packaging-задачей после стабилизации MVP.
