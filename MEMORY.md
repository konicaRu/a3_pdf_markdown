...
## 2026-07-14 PDF: структурное извлечение таблиц

- В `PdfPipeline._convert_page` добавлен приоритетный шаг: `page.find_tables()` (PyMuPDF).
  Если на странице найдены таблицы, они конвертируются напрямую в Markdown-таблицы, текст
  вне таблиц вытаскивается отдельно по bbox-фильтру (`_extract_text_outside_tables`); OCR и
  vision для такой страницы не используются.
- `ARCHITECTURE.md` синхронизирован с этим изменением (шаги PDF pipeline и раздел «Реализовано»).
- Локальные коммиты `edeb8ff` (реализация) и `8803fef` (обновление ARCHITECTURE.md)
  были ранее не запушены; сегодня запушены в `https://github.com/konicaRu/a3_pdf_markdown.git`.
