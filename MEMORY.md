...
- После уточнения: fallback без LLM для PPTX должен сохранять описания картинок. Добавлен отдельный `PptxEnricher`, который извлекает embedded-картинки через `python-pptx`, описывает их vision-моделью и добавляет описания в единый `.md`.
